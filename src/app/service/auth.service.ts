...
export class AuthService {
  ...
  isVendorInSubject = new BehaviorSubject<boolean>(false);

  isAdmin$: Observable<boolean> = this.isAdminSubject.asObservable();
  isLoggedIn$: Observable<boolean> = this.isLoggedInSubject.asObservable();
  isVendor$: Observable<boolean> = this.isVendorInSubject.asObservable();
  ...
  isVendor(): boolean{
    return this.isVendorInSubject.value;
  }
  ...
  setLoggedIn(user: AppUser): void {
    ...
    else if(user.role === CONSTANT.VENDOR){
      this.isVendorInSubject.next(true);
      this.router.navigate(['/vendorhome'],{replaceUrl:true})
    }
  }

  register(newregister:Register):Observable<AppResponse>{
    return this.http.post<AppResponse>(
      `${urlEndpoint.baseUrl}/auth/register`,newregister
    );
